                ...
                onTap: () async {
                  if (_emailTextEditingController.text.trim().isNotEmpty &&
                    ...
                    try {
                      ...
                      log("C@W : UserCredentials :${userCredential.user!.uid}");
                      String userCred = userCredential.user!.uid;
                      QuerySnapshot response = await FirebaseFirestore.instance
                          .collection("User")
                          .get();

                      for (int i = 0; i < response.docs.length; i++) {
                        if (userCred == response.docs[i]['userId']) {
                          log(response.docs[i]['name']);
                          fetchedName = response.docs[i]['name'];
                          //Shared Preference store "Name"
